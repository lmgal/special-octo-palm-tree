...
def plot2d_animate(ca, title='', *, colormap='Greys', show_grid=False, show_margin=True, scale=0.6, dpi=80,
    ...
    def updatefig(*args):
        ...
        if autoscale:
            im.autoscale()
        return im, grid
    ani = anime.FuncAnimation(fig, updatefig, interval=interval, blit=True, save_count=len(ca))
    if save:
...
def custom_rule(neighbourhood, c, t):
    center_cell = neighbourhood[1][1]
    total = np.sum(neighbourhood)
    if center_cell == 1:
        if neighbourhood[2][1] == 0:
            return 0
...
cellularAutomaton[:, [1,2], [3,3]] = 1
cellularAutomaton[:, [2,2], [4,5]] = 2
cellularAutomaton[:, [3], [2]] = 3 # can be changed
